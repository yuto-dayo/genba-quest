...
export function classifyTransitionalPhase(date: Date | string): TransitionalPhase {
  ...
  if (isoDate < PHASE1_START) {
    return "pre-introduction";
  }
  if (isoDate < PHASE2_START) {
    return "phase1-80";
  }
  if (isoDate < PHASE3_START) {
    return "phase2-50";
  }
  return "phase3-0";
}
...
export function calculateTransitionalDeductionRate(
  ...
  if (status === "registered" || status === "unknown") {
    return 1;
  }

  const phase = classifyTransitionalPhase(date);
  if (phase === "pre-introduction") {
    return 1;
  }
  if (phase === "phase1-80") {
    return 0.8;
...
export function nextTransitionalRateChange(date: Date | string): {
  ...
  const isoDate = toIsoDate(date);
  if (isoDate < PHASE1_START) {
    return { date: PHASE1_START, fromRate: 1, toRate: 0.8 };
  }
  if (isoDate < PHASE2_START) {
    return { date: PHASE2_START, fromRate: 0.8, toRate: 0.5 };
  }
  if (isoDate < PHASE3_START) {
    return { date: PHASE3_START, fromRate: 0.5, toRate: 0 };
  }
  return null;
}
