...
- [ ] タブ名を「取引先」→「取引先・請求書」に変更
- [ ] タブ上部に件数フィルタチップ: `[期限超過 N][今週入金予定 N][下書き N][全部]`
- [ ] 各取引先カードに直近1件の請求書状態(MoneyMock.tsx の partnerCard 構造)
- [ ] 期限超過カードは左に 3px 縦バー(`--money-status-overdue`)
- [ ] カードタップ → 右ドロワー(取引先詳細)open
- [ ] ドロワーに過去全請求書のタイムライン + `[入金を記録]` ボタン
- [ ] **`[+ 新規請求書を発行]` ボタンは置かない**(FAB一本化、案Y)
- [ ] tsc/lint/test グリーン
