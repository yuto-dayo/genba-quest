-- ============================================================
-- 059: PATH canonical reward writer cutover foundation
-- ============================================================
-- 目的:
--   1) fixed month_close_id 配下に reward basis snapshot を固定する
--   2) cached reward preview / receipt を canonical に保持する
--   3) payout posting を canonical reward_run に直接ひも付ける
--   4) reward.calculate の active proposal を month_close 単位で 1 本に制約する
-- ============================================================
...
DROP POLICY IF EXISTS "Insert reward_basis_snapshots" ON public.reward_basis_snapshots;
DROP POLICY IF EXISTS "Update reward_basis_snapshots" ON public.reward_basis_snapshots;
...
CREATE POLICY "Insert reward_basis_snapshots"
ON public.reward_basis_snapshots
...
CREATE POLICY "Update reward_basis_snapshots"
ON public.reward_basis_snapshots
...
DROP POLICY IF EXISTS "Insert reward_basis_member_snapshots" ON public.reward_basis_member_snapshots;
DROP POLICY IF EXISTS "Update reward_basis_member_snapshots" ON public.reward_basis_member_snapshots;
...
CREATE POLICY "Insert reward_basis_member_snapshots"
ON public.reward_basis_member_snapshots
FOR INSERT TO authenticated
WITH CHECK (
  org_id = COALESCE(
...
CREATE POLICY "Update reward_basis_member_snapshots"
ON public.reward_basis_member_snapshots
FOR UPDATE TO authenticated
USING (
  org_id = COALESCE(
...
DROP POLICY IF EXISTS "Insert reward_basis_package_snapshots" ON public.reward_basis_package_snapshots;
DROP POLICY IF EXISTS "Update reward_basis_package_snapshots" ON public.reward_basis_package_snapshots;
...
CREATE POLICY "Insert reward_basis_package_snapshots"
ON public.reward_basis_package_snapshots
FOR INSERT TO authenticated
WITH CHECK (
  org_id = COALESCE(
...
CREATE POLICY "Update reward_basis_package_snapshots"
ON public.reward_basis_package_snapshots
FOR UPDATE TO authenticated
USING (
  org_id = COALESCE(
...
DROP POLICY IF EXISTS "Insert reward_preview_snapshots" ON public.reward_preview_snapshots;
DROP POLICY IF EXISTS "Update reward_preview_snapshots" ON public.reward_preview_snapshots;
...
CREATE POLICY "Insert reward_preview_snapshots"
ON public.reward_preview_snapshots
FOR INSERT TO authenticated
WITH CHECK (
  org_id = COALESCE(
...
CREATE POLICY "Update reward_preview_snapshots"
ON public.reward_preview_snapshots
FOR UPDATE TO authenticated
USING (
  org_id = COALESCE(
...
DROP POLICY IF EXISTS "Insert reward_run_receipts" ON public.reward_run_receipts;
DROP POLICY IF EXISTS "Update reward_run_receipts" ON public.reward_run_receipts;
...
CREATE POLICY "Insert reward_run_receipts"
ON public.reward_run_receipts
FOR INSERT TO authenticated
WITH CHECK (
  org_id = COALESCE(
...
CREATE POLICY "Update reward_run_receipts"
ON public.reward_run_receipts
FOR UPDATE TO authenticated
USING (
  org_id = COALESCE(
...
ALTER TABLE public.reward_runs
  ADD COLUMN IF NOT EXISTS policy_bundle_version_id uuid REFERENCES public.policy_bundle_versions(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS policy_fingerprint text,
  ADD COLUMN IF NOT EXISTS reward_engine_version text,
  ADD COLUMN IF NOT EXISTS rounding_mode text,
  ADD COLUMN IF NOT EXISTS rounding_scale integer,
  ADD COLUMN IF NOT EXISTS rounding_minor_unit integer,
  ADD COLUMN IF NOT EXISTS input_hash text,
  ADD COLUMN IF NOT EXISTS preview_snapshot_id uuid REFERENCES public.reward_preview_snapshots(id) ON DELETE SET NULL,
...
ALTER TABLE public.finance_payout_postings
  ADD COLUMN IF NOT EXISTS canonical_reward_run_id uuid REFERENCES public.reward_runs(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS posting_group_id uuid REFERENCES public.posting_groups(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS finance_payout_postings_canonical_reward_run_idx
  ON public.finance_payout_postings (canonical_reward_run_id)
  WHERE canonical_reward_run_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS finance_payout_postings_posting_group_idx
  ON public.finance_payout_postings (posting_group_id)
  WHERE posting_group_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS proposals_reward_calculate_active_once
  ON public.proposals (org_id, month_close_id, reward_rule_version_id)
  WHERE type = 'reward.calculate'
    AND month_close_id IS NOT NULL
    AND reward_rule_version_id IS NOT NULL
    AND status IN ('draft', 'pending', 'approved', 'executed');
