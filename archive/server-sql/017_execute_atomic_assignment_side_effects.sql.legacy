-- ============================================================
-- execute_proposal_atomic 拡張: assignment.create の副作用を原子実行
-- ============================================================
-- 目的:
--   atomic RPC 経路でも assignment.create のドメイン更新
--   (sites.assigned_users / profiles.current_site_id) を実行し、
--   fallback 経路との挙動差分をなくす
--
-- 適用順:
--   013 -> 014 -> 015 -> 016 -> 017
-- ============================================================
...
COMMENT ON FUNCTION public.execute_proposal_atomic IS
  'Proposal実行を原子的に実行: Event作成 + 仕訳生成 + assignment.create副作用 + ステータス更新を1トランザクションで';
