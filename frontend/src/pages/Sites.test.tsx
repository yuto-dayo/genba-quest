...
describe("Sites", () => {
    ...
    it("returns to LUQO when a site detail opened from PATH is closed", async () => {
        fetchSites.mockResolvedValue([baseSite]);
        fetchSite.mockResolvedValue(baseSite);
        ...
        render(
            <MemoryRouter initialEntries={["/sites?site=site-1&return=luqo&period=2026-04&reward=1&member=member-1"]}>
                <Routes>
                    <Route path="/sites" element={<Sites />} />
                    <Route path="/luqo" element={<div data-testid="luqo-page">LUQO</div>} />
                </Routes>
            </MemoryRouter>,
        );
        ...
        expect(await screen.findByTestId("luqo-page")).toBeInTheDocument();
    });
});
