-- P1 canonical no-PL-revenue posting for Money payment allocations.
--
-- Allocation consumes unapplied cash/customer deposit and clears AR without
-- increasing PL revenue.
...
BEGIN
  ...
  WHERE execution.org_id = p_org_id
    AND execution.idempotency_key = 'accounting.payments.allocate:' || p_idempotency_key
  FOR UPDATE;
...
REVOKE ALL ON FUNCTION public.rpc_allocate_accounting_payment_canonical(
  uuid,
  uuid,
  uuid,
  text,
...
GRANT EXECUTE ON FUNCTION public.rpc_allocate_accounting_payment_canonical(
  uuid,
  uuid,
  uuid,
  text,
...
COMMENT ON FUNCTION public.rpc_allocate_accounting_payment_canonical(
  uuid,
  uuid,
  uuid,
  text,
  ...
  jsonb,
  text
) IS 'Creates transition payment.allocate lineage, proposal execution, payment allocation posting group, balanced no-PL-revenue journal, and payment allocation projection metadata.';
